...
export async function submitDriverApplication(formData: FormData) {
  ...
  if (!vehicleCategory || !vehicleModel || !Number.isInteger(passengers) || passengers < 1) {
    redirect("/driver/signup?error=Please%20provide%20your%20vehicle%20type%2C%20make%2Fmodel%20and%20passenger%20capacity.");
  }
  ...
  const { data: authData, error: authError } = await supabaseAdmin.auth.admin.createUser({
    ...
    password,
    email_confirm: false,
    user_metadata: { full_name: fullName, phone, account_type: "driver" },
  });

  if (authError || !authData.user) {
    redirect(`/driver/signup?error=${encodeURIComponent(authError?.message ?? "Unable to create account.")}`);
  }
  ...
  try {
    const { data: driver, error: driverError } = await supabaseAdmin
      ...
      .insert({
        ...
        service_country: country || null,
        service_city: city,
        service_airport_code: airport || null,
        source: "safariplug",
      })
      .select("id")
      .single();
    ...
    const { error: vehicleError } = await supabaseAdmin.from("vehicles").insert({
      ...
      luggage_capacity: Number.isInteger(luggage) && luggage >= 0 ? luggage : null,
      accessibility: selectedCapabilities.includes("wheelchair_accessible"),
      status: "draft",
    });
    if (vehicleError) throw new Error(vehicleError.message);

    if (availableOn) {
      const { error: availabilityError } = await supabaseAdmin.from("driver_availability").insert({
        driver_id: driver.id,
    ...
    const { error: verificationError } = await supabaseAdmin.from("verification_cases").insert({
      subject_type: "driver",
      subject_id: driver.id,
      status: "not_started",
      verification_level: "enhanced",
      provider: "human_review",
      notes: "Driver onboarding submitted. Live face/liveness verification is mandatory before approval and booking eligibility.",
    });
