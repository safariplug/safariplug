...
export default function AppointmentsPage() {
  ...
  async function load() {
    const r = await fetch("/api/account/appointments", { cache:"no-store" });
    const j = await r.json();
    if (!r.ok) { setMessage(j.error || "Please sign in to view your appointments."); return; }
    setAppointments(j.appointments || []); setEvents(j.events || []);
  }
  useEffect(()=>{ load(); },[]);

  const upcoming = useMemo(()=>appointments.filter(a => new Date(a.starts_at) >= new Date() && !["cancelled","completed","no_show"].includes(a.status)),[appointments]);
  const past = useMemo(()=>appointments.filter(a => !upcoming.some(x=>x.id===a.id)),[appointments,upcoming]);
  ...
  async function saveReschedule() {
    ...
    setBusy(true); setMessage("");
    const r=await fetch("/api/account/appointments",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({action:"reschedule",appointmentId:selected.id,startsAt:new Date(reschedule).toISOString()})});
    const j=await r.json(); setBusy(false); setMessage(r.ok?"Appointment rescheduled.":(j.error||"Unable to reschedule.")); if(r.ok){setSelected(j.appointment);setReschedule("");await load();}
  }

  if(message && !appointments.length) return <main className="min-h-screen bg-[#f7f7f4] px-6 py-16 text-[#111]"><div className="mx-auto max-w-3xl rounded-[2rem] bg-white p-8 text-center shadow-sm"><p className="text-sm text-black/55">{message}</p><Link href="/login?next=/account/appointments" className="mt-5 inline-flex rounded-xl bg-black px-5 py-3 text-sm font-semibold text-white">Sign in</Link></div></main>;

  return <main className="min-h-screen bg-[#f7f7f4] text-[#111]">
    <header className="border-b border-black/8 bg-white"><div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-5 sm:px-10"><div><Link href="/" className="text-sm font-semibold">SafariPlug</Link><p className="mt-1 text-[10px] uppercase tracking-[.25em] text-black/35">My appointments</p></div><Link href="/services" className="rounded-xl border border-black/10 px-4 py-2 text-xs font-semibold">Book a service</Link></div></header>
