...
export default async function AdminHome() {
  ...
  const [prospects, invites, suppliers, payouts, events, travelerVerification, serviceRefunds, overdueServices] = await Promise.all([
    ...
    supabaseAdmin.from("service_appointments").select("id",{count:"exact",head:true}).in("status",["pending","confirmed","checked_in","in_progress"]).lt("ends_at",new Date().toISOString()),
  ]);
  ...
  const serviceOpsAttention = Number(serviceRefunds.count||0) + Number(overdueServices.count||0);
  const attention = inviteActions.length + supplierActions.length + payoutActions.length + e.length + travelerTrustActions.length + serviceOpsAttention;
  const queryErrors = [prospects.error, invites.error, suppliers.error, payouts.error, events.error, travelerVerification.error, serviceRefunds.error, overdueServices.error].filter(Boolean);
  ...
  return <main className="min-h-screen bg-[#070707] px-5 py-10 text-white md:px-10"><div className="mx-auto max-w-7xl">
    ...
    <section className="mt-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7"><Metric label="Recent AI prospects" value={p.length} href="/admin/crm"/><Metric label="Invites need action" value={inviteActions.length} href="/admin/ai-sales/invitations"/><Metric label="Supplier onboarding" value={supplierActions.length} href="/admin/suppliers"/><Metric label="Service operations" value={serviceOpsAttention} href="/admin/services/operations"/><Metric label="Payout actions" value={payoutActions.length} href="/admin/payouts"/><Metric label="Events pending" value={e.length} href="/admin/ai-events"/><Metric label="Traveler verification" value={travelerTrustActions.length} href="/admin/integrations/verification"/></section>

    <section className="mt-8"><div className="mb-4"><h2 className="text-xl font-semibold">Action queue</h2><p className="mt-1 text-sm text-zinc-500">Human-governed work that should be reviewed before external or financial action.</p></div><div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-6">
      ...
      <Queue title="Service operations" href="/admin/services/operations" count={serviceOpsAttention} text="Overdue active service appointments and open paid-cancellation finance reviews."/><Queue title="Provider payouts" href="/admin/payouts" count={payoutActions.length} text="Eligible, approved, processing or held payout records."/>
    ...
    <section className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-3"><Module label="Marketplace operations" title="Service Operations" text="Triage overdue appointments, refund reviews, payout exceptions and service-provider trust blockers in one view." href="/admin/services/operations"/><Module label="Growth" title="AI CRM" text="Discover prospects, approve outreach, invite partners, track enrollment and open Partner 360." href="/admin/crm"/><Module label="Finance" title="Accounting & Commission" text="Sales, commissions, payables, payouts, refunds, reconciliation and reports from recorded money." href="/admin/accounting"/><Module label="Partner operations" title="Partner 360 & Enrollment" text="Onboarding, verification, activation readiness and supplier relationships." href="/admin/ai-sales/partners"/><Module label="Curation" title="Event Discovery & Approval" text="Review AI-discovered events, verify details and approve listings." href="/admin/ai-events"/><Module label="Marketing" title="Amani Studio" text="Create human-reviewed promotional drafts for SafariPlug campaigns." href="/admin/marketing"/><Module label="Marketplace" title="Supplier Review" text="Review supplier onboarding and governed marketplace activation." href="/admin/suppliers"/><Module label="Recruitment" title="Invite Supplier" text="Manually add a known supplier and create a governed invitation draft." href="/admin/suppliers/invite"/><Module label="Payouts" title="Provider Payouts" text="Review provider earnings and governed payout status." href="/admin/payouts"/><Module label="Integrations" title="Travel Integrations" text="Aurelian, hotel, transfer and provider integration foundations." href="/admin/integrations"/><Module label="Trust & safety" title="Verification Operations" text="Monitor traveler, driver, Local and provider identity/liveness cases without manually inventing external verification approval." href="/admin/integrations/verification"/><Module label="Intelligence" title="AI Event Scout" text="Run and review SafariPlug event discovery intelligence." href="/admin/ai-scout"/></section>
