...
export default async function AdminHome() {
  ...
  const [prospects, invites, suppliers, payouts, events] = await Promise.all([
  ...
  const attention = inviteActions.length + supplierActions.length + payoutActions.length + e.length;
  const queryErrors = [prospects.error, invites.error, suppliers.error, payouts.error, events.error].filter(Boolean);
  ...
  return <main className="min-h-screen bg-[#070707] px-5 py-10 text-white md:px-10"><div className="mx-auto max-w-7xl">
    ...
    <section className="mt-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-5"><Metric label="Recent AI prospects" value={p.length} href="/admin/crm"/><Metric label="Invites need action" value={inviteActions.length} href="/admin/ai-sales/invitations"/><Metric label="Supplier onboarding" value={supplierActions.length} href="/admin/suppliers"/><Metric label="Payout actions" value={payoutActions.length} href="/admin/payouts"/><Metric label="Events pending" value={e.length} href="/admin/ai-events"/></section>

    <section className="mt-8"><div className="mb-4"><h2 className="text-xl font-semibold">Action queue</h2><p className="mt-1 text-sm text-zinc-500">Human-governed work that should be reviewed before external or financial action.</p></div><div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-4">
      ...
      <Queue title="Event curation" href="/admin/ai-events" count={e.length} text="AI-discovered events waiting for human review."/>
    </div></section>
