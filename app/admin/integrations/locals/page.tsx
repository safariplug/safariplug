...
export default async function AdminLocalsPage() {
  ...
  let locals = data ?? [];
  if (error) {
    const fallback = await supabaseAdmin.from("local_profiles").select("id,display_name,personal_photo_url,city,country,languages,interests,hourly_rate,currency,verification_state,service_status,created_at").order("created_at", { ascending: false });
  ...
  return <main className="min-h-screen bg-[#050505] p-8 text-white"><div className="mx-auto max-w-7xl space-y-8">
    ...
    <header className="border-b border-zinc-800 pb-6"><p className="font-mono text-[11px] font-bold uppercase tracking-widest text-amber-400">Local operations</p><h1 className="mt-2 text-3xl font-extrabold">Identity review & activation</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-zinc-400">Human-governed control surface. Verification approval and marketplace activation are separate actions. No Local becomes publicly discoverable until identity is verified and service status is active.</p></header>
