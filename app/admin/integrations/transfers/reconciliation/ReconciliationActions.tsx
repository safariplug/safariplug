...
export default function ReconciliationActions({
  ...
  async function act(action: "verify_reference" | "mark_not_booked") {
    ...
    setError("");
    try {
      const response = await fetch("/api/admin/integrations/hotelbeds/transfers-reconciliation", {
        method: "POST",
    ...
    <div className="mt-5 grid gap-4 lg:grid-cols-2">
      <div className="rounded-xl border border-zinc-800 bg-black p-4">
        ...
        <p className="mt-2 text-xs leading-5 text-zinc-500">
          Enter a supplier booking reference only after you have evidence a booking may exist.
          This makes one explicit Hotelbeds booking-retrieval request. It does not create or retry a booking.
        </p>
        <input
          value={reference}
        ...
        >
          {busy === "verify" ? "Verifying…" : "Verify reference once"}
        </button>
      </div>
      ...
      <div className="rounded-xl border border-zinc-800 bg-black p-4">
        ...
        <p className="mt-2 text-xs leading-5 text-zinc-500">
          Use only after staff have verified no Hotelbeds booking exists. Payment remains recorded as paid
          and the case is flagged for manual refund handling.
        </p>
        <input
          value={confirmation}
      ...
      <p className="font-mono text-[10px] text-zinc-700 lg:col-span-2">Ledger {ledgerId} · {preparedBookingId}</p>
    </div>
  );
}
