...
type Supplier = { id: string; contact_name: string; invitation_status: string; onboarding_status: string; completion_percent: number; submitted_at: string | null; approved_at?: string | null; created_at?: string | null; businesses?: Business | Business[] | null };
...
function guidance(supplier: Supplier) {
  ...
  if (status === "changes_requested") return { owner: "supplier", title: "Waiting for changes", detail: "Supplier must update and resubmit.", priority: 1 };
...
export default function SuppliersAdminPage() {
  ...
  async function review(supplierId: string, action: "approve" | "request_changes" | "reject") {
    ...
    const data = await response.json().catch(() => null);
    setMessage(response.ok ? (action === "approve" ? "Supplier approved and published." : action === "request_changes" ? "Supplier returned for changes." : "Supplier rejected.") : data?.error || "Unable to update supplier.");
    setReviewing("");
