...
export default async function AISalesPage({
  ...
  const outreachReady = allProspects.filter((p) => p.review_status === "approved" && p.status !== "rejected" && Boolean(p.contact_email)).length;
            ...
            {stage === "approved" && outreachReady > 0 ? (
              <div className="mb-5 flex flex-col justify-between gap-4 rounded-xl border border-emerald-200 bg-emerald-50 p-4 md:flex-row md:items-center">
                <div>
                  ...
                  <p className="mt-1 font-semibold text-emerald-950">{outreachReady} approved prospect{outreachReady === 1 ? "" : "s"} have a business email ready for governed outreach.</p>
