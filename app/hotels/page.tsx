...
function hotelImage(src?: string) { return src ? `/api/media/hotel?url=${encodeURIComponent(src)}` : ""; }
...
export default function HotelsPage() {
  ...
  async function search(event: FormEvent) {
    ...
    try {
      const params = new URLSearchParams({ destination: destination.trim(), check_in: checkIn, check_out: checkOut, guests, rooms, currency: "KES", location_scope: "specific", bookable_only: "true" });
  ...
  return <main className="min-h-screen bg-[#f7f7f4] text-[#111]">
    ...
    <section className="mx-auto max-w-7xl px-6 py-8 sm:px-10">
      ...
      {!results.length && !loading && !error && <div className="mt-6 rounded-[2rem] border border-dashed border-black/15 bg-white px-6 py-16 text-center"><p className="text-lg font-semibold">{searched ? "No live rooms matched that search." : "Search any destination."}</p><p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-black/50">SafariPlug only shows supplier-confirmed, bookable room inventory for your dates; no placeholder hotel rates.</p></div>}
