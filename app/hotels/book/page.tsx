    ...
    <div className="mx-auto max-w-5xl">
      ...
      <div className="mt-6 grid gap-8 lg:grid-cols-[1.15fr_.85fr]">
        ...
        <aside>{selected ? <form onSubmit={checkout} className="rounded-3xl bg-white p-6 shadow-sm lg:sticky lg:top-6">
          ...
          <div className="mt-4 rounded-2xl bg-black/[.035] p-4"><p className="text-xs text-black/45">Booking total</p><p className="mt-1 text-3xl font-semibold">{selected.customerCurrency || currency} {Number(selected.price).toLocaleString(undefined,{maximumFractionDigits:2})}</p><p className="mt-2 text-xs leading-5 text-black/45">Total shown for your booking. Final availability and amount are revalidated against the supplier before M-Pesa payment starts.</p></div>
