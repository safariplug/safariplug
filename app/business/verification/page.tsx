...
export default async function ProviderVerificationPage() {
  ...
  const automatedReady = Boolean(identity?.contract_implemented && identity?.configured && liveness?.contract_implemented && liveness?.configured);
  ...
  return (
    <main className="min-h-screen bg-[#f7f7f4] text-[#111]">
      ...
      <section className="mx-auto max-w-6xl px-6 py-12 sm:px-10"><div className="max-w-3xl"><p className="text-[11px] font-semibold uppercase tracking-[.25em] text-black/40">Provider verification</p><h1 className="mt-3 text-4xl font-semibold tracking-[-.04em] sm:text-6xl">Trust before payout.</h1><p className="mt-4 text-base leading-7 text-black/50">SafariPlug requires a completed provider trust review before payouts can be released. At launch, SafariPlug staff can review the provider account manually; automated identity + liveness can be added later when it makes financial sense.</p></div>
