...
export default function VerificationStart({ hasCase, status }: { hasCase: boolean; status: string | null }) {
  ...
  async function start() {
    ...
    try {
      ...
      window.setTimeout(() => {
        const instance = sdk.init(body.accessToken, async () => { const refresh = await fetch("/api/business/verification/session", { method: "POST" }); const refreshed = await refresh.json(); if (!refresh.ok || typeof refreshed.accessToken !== "string") throw new Error("Unable to refresh verification session."); return refreshed.accessToken; }).withConf({ lang: "en", theme: "light", email: undefined }).withOptions({ addViewportTag: false, adaptIframeHeight: true }).on("idCheck.onError", (error: unknown) => setMessage(typeof error === "string" ? error : "Verification session reported an error.")).onMessage((type: string, payload: unknown) => {
          void payload;
