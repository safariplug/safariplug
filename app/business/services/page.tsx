...
export default async function BusinessServicesDashboard() {
  ...
  if (business) {
    const { data } = await supabaseAdmin.from("service_profiles").select("id,business_id,category_id,status,booking_status,timezone,cancellation_policy,booking_notice_minutes,max_booking_days,provider_terms_version,provider_terms_accepted_at,provider_terms_accepted_by,service_fee_percent,service_categories(id,name,slug,description)").eq("business_id",business.id).maybeSingle();
    profile = data;
