...
async function supplierContext() {
  ...
  const { data: account } = await supabaseAdmin
    ...
    .select("id,user_id,business_id,contact_name,onboarding_status")
    .eq("user_id", user.id)
    .maybeSingle();
  if (!account) return null;
  const { data: profile } = await supabaseAdmin
    .from("service_profiles")
    .select("id,timezone,booking_status")
...
export async function GET(request: Request) {
  ...
  const { data: staff, error: staffError } = await supabaseAdmin
    ...
    .eq("service_profile_id", ctx.profile.id)
    .order("created_at");
  ...
  const [availabilityResult, blockoutResult, appointmentsResult] = await Promise.all([
    staffIds.length
      ? supabaseAdmin.from("service_staff_availability").select("id,staff_id,day_of_week,start_time,end_time,is_active").in("staff_id", staffIds).order("day_of_week").order("start_time")
      : Promise.resolve({ data: [], error: null }),
    staffIds.length
      ? supabaseAdmin.from("service_staff_blockouts").select("id,staff_id,starts_at,ends_at,reason").in("staff_id", staffIds).lt("starts_at", to).gt("ends_at", from).order("starts_at")
      : Promise.resolve({ data: [], error: null }),
    staffIds.length
      ? supabaseAdmin.from("service_appointments").select("id,public_id,staff_id,customer_name,starts_at,ends_at,status,service_offerings(name)").eq("service_profile_id", ctx.profile.id).in("staff_id", staffIds).lt("starts_at", to).gt("ends_at", from).in("status", ["pending","confirmed","checked_in","in_progress"]).order("starts_at")
      : Promise.resolve({ data: [], error: null }),
  ]);
...
export async function POST(request: Request) {
  ...
  if (action === "availability") {
    ...
    if (!Number.isInteger(day) || day < 0 || day > 6 || !/^([01]\d|2[0-3]):[0-5]\d$/.test(start) || !/^([01]\d|2[0-3]):[0-5]\d$/.test(end) || start >= end) {
      return NextResponse.json({ error: "Use a valid day and start/end time." }, { status: 400 });
    }
