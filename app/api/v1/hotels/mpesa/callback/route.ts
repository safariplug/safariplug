...
export async function POST(request: Request) {
  try {
    ...
    const { data: ledger } = await supabaseAdmin.from("hotel_booking_pricing_ledger").select("id,metadata,payment_status,booking_status").eq("payment_provider", "mpesa").eq("payment_reference", checkoutRequestId).maybeSingle();
    ...
    await supabaseAdmin.from("hotel_booking_pricing_ledger").update({
    ...
    }).eq("id", ledger.id).eq("payment_provider", "mpesa").eq("payment_reference", checkoutRequestId);
