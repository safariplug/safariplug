...
export async function POST() {
  ...
  if (!fullName || !email || !businessName || !businessType) return NextResponse.json({ error: "Partner registration details are incomplete. Please start registration again." }, { status: 422 });
  ...
  if (categoryName) {
    const { data: category } = await supabaseAdmin.from("service_categories").select("id").eq("name", categoryName).eq("status", "active").maybeSingle();
    if (category) {
