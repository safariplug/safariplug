...
export async function POST(request: Request) {
  try {
    ...
    const { data: category, error: categoryError } = await supabaseAdmin.from("service_categories").select("id,name").eq("name", businessType).eq("status", "active").maybeSingle();
    if (categoryError) return NextResponse.json({ success: false, error: categoryError.message }, { status: 500 });
    if (!category) return NextResponse.json({ success: false, error: "That business type is not currently available." }, { status: 422 });

    const { data: authData, error: authError } = await supabaseAdmin.auth.admin.createUser({ email, password, email_confirm: false, user_metadata: { full_name: fullName, phone, account_type: "supplier" } });
    ...
    try {
      ...
      const { data: business, error: businessError } = await supabaseAdmin.from("businesses").insert({ owner_id: userId, name: businessName, slug, business_type: businessType, phone, whatsapp: phone, email, status: "pending", verified: false, claimed: true }).select("id").single();
      ...
      const { data: linkData, error: linkError } = await supabaseAdmin.auth.admin.generateLink({ type: "invite", email, options: { redirectTo: `${appUrl}/auth/confirm` } });
