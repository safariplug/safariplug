...
export async function POST(request: Request) {
  try {
    ...
    if (!fullName || !email || !emailPattern.test(email) || !phone || !businessName || !businessType || password.length < 8) {
      return NextResponse.json({ success: false, error: "Please complete all fields and use a password of at least 8 characters." }, { status: 400 });
    }
    ...
    if (existingUsers?.users.some((user) => user.email?.toLowerCase() === email)) {
      return NextResponse.json({ success: false, error: "An account already exists for this email. Please sign in instead." }, { status: 409 });
    }
    ...
    const { data: authData, error: authError } = await supabaseAdmin.auth.admin.createUser({
      ...
      password,
      email_confirm: false,
      user_metadata: { full_name: fullName, phone, account_type: "supplier" },
    });
    if (authError || !authData.user) {
      return NextResponse.json({ success: false, error: authError?.message ?? "Unable to create your account." }, { status: 500 });
    }
    ...
    try {
      ...
      const { error: emailError } = await resend.emails.send({
        ...
        to: email,
        subject: "Confirm your SafariPlug business account",
    ...
    return NextResponse.json({ success: false, error: error instanceof Error ? error.message : "Signup failed." }, { status: 500 });
  }
}
