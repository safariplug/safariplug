...
export async function POST(request: Request) {
  try {
    ...
    const resultCode = Number(callback?.ResultCode); const metadata = Array.isArray(callback?.CallbackMetadata?.Item) ? callback.CallbackMetadata.Item : [];
    const receipt = metadata.find((i:{Name?:string})=>i.Name === "MpesaReceiptNumber")?.Value; const amount = metadata.find((i:{Name?:string})=>i.Name === "Amount")?.Value;
    ...
    if (foodIdem?.order_id) {
      await supabaseAdmin.from("food_orders").update({ payment_status: resultCode === 0 ? "paid" : "failed", payment_reference: receipt ? String(receipt) : checkoutRequestId, paid_at: resultCode === 0 ? new Date().toISOString() : null }).eq("id",foodIdem.order_id);
      return NextResponse.json({ ResultCode: 0, ResultDesc: "Accepted" });
    }

    const { data: idem } = await supabaseAdmin.from("service_payment_idempotency").select("appointment_id").eq("provider","mpesa").eq("provider_reference",checkoutRequestId).maybeSingle();
