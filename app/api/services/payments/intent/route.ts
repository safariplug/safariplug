...
export async function POST(request: Request) {
  try {
    ...
    const status = message.includes("not_configured") ? 503
      ...
      : message.includes("appointment_not_payable") || message.includes("appointment_payment_settled") || message.includes("payment_reconciliation_required") || message.includes("payment_attempt_terminal_retry_required") || message.includes("idempotency_key_reused") || message.includes("payment_intent_in_progress") || message.includes("payment_intent_submission_uncertain") ? 409
      : 400;
