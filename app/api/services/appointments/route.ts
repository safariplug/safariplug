...
async function notifySupplier(params: { serviceProfileId: string; offeringId: string; customerName: string; customerEmail?: string | null; customerPhone?: string | null; startsAt: string }) {
  ...
  const message = [
    ...
    "Open SafariPlug to manage this booking.",
  ].filter(Boolean).join("\n");
  ...
  if (supplier?.user_id) {
    ...
    if (tokens?.length) {
      const result = await sendExpoPushNotifications(tokens.map((row) => row.expo_push_token), {
        title: `New booking · ${serviceName}`,
        body: `${params.customerName} booked ${when}. Tap to manage it.`,
        data: { type: "service_booking", serviceProfileId: params.serviceProfileId },
      });
      push = result.sent;
    }
  }
...
export async function POST(request: Request) {
  try {
    ...
    const notifications = await notifySupplier({ serviceProfileId: b.serviceProfileId, offeringId: b.offeringId, customerName: b.customerName, customerEmail: b.customerEmail ?? null, customerPhone: b.customerPhone ?? null, startsAt: b.startsAt });
