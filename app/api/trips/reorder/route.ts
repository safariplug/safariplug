...
export async function PATCH(request: Request) {
  ...
  const { error } = await supabaseAdmin.rpc("reorder_trip_items", {
    p_trip_id: tripId,
  ...
  if (error) {
    ...
    if (error.message.includes("item_not_in_trip")) return NextResponse.json({ error: "One or more itinerary items do not belong to this journey." }, { status: 400 });
    if (error.message.includes("duplicate_item_ids")) return NextResponse.json({ error: "Duplicate itinerary items are not allowed." }, { status: 400 });
    if (error.message.includes("items_required")) return NextResponse.json({ error: "tripId and itemIds are required." }, { status: 400 });
