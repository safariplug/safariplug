...
export async function GET() {
  try {
    ...
    const [prospectsResult, partnersResult, invitationsResult, suppliersResult, followupsResult, schedulerResult] = await Promise.all([
      supabaseAdmin.from("ai_sales_prospects").select("id,business_name,category,city,status,review_status,contact_email,phone,created_at").eq("review_status","approved").order("updated_at",{ascending:false}).limit(100),
      ...
      supabaseAdmin.from("partner_invitations").select("id,business_name,partner_type,status,contact_email,whatsapp_phone,prospect_id,created_at,opened_at,signup_started_at,onboarded_user_id").order("created_at", { ascending: false }).limit(50),
