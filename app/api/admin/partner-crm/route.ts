...
export async function GET() {
  try {
    await requireAdmin();
    const [partnersResult, invitationsResult, suppliersResult, followupsResult, schedulerResult] = await Promise.all([
    ...
    const error = partnersResult.error || invitationsResult.error || suppliersResult.error || followupsResult.error || schedulerResult.error;
    ...
    return NextResponse.json({ partners: partnersResult.data ?? [], invitations: invitationsResult.data ?? [], suppliers, scheduler: schedulerResult.data ?? null });
  } catch (error) {
