...
export async function GET() {
  try {
    await requireAdmin();
    const [partnersResult, invitationsResult, suppliersResult, followupsResult] = await Promise.all([
    ...
    const error = partnersResult.error || invitationsResult.error || suppliersResult.error || followupsResult.error;
    ...
    return NextResponse.json({ partners: partnersResult.data ?? [], invitations: invitationsResult.data ?? [], suppliers });
  } catch (error) {
