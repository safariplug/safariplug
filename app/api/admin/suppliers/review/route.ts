...
export async function POST(request: Request) {
  try {
    ...
    if (!supplierId || !["approve", "request_changes", "reject"].includes(action)) return NextResponse.json({ error: "Supplier and valid review action are required." }, { status: 400 });

    const { data: account, error: accountError } = await supabaseAdmin.from("supplier_accounts").select("id,business_id,onboarding_status").eq("id", supplierId).maybeSingle();
    if (accountError) return NextResponse.json({ error: accountError.message }, { status: 500 });
    if (!account) return NextResponse.json({ error: "Supplier not found." }, { status: 404 });
    if (!["submitted", "changes_requested"].includes(account.onboarding_status)) return NextResponse.json({ error: "This supplier is not awaiting review." }, { status: 409 });

    if (action === "approve") {
      const now = new Date().toISOString();
      const { error } = await supabaseAdmin.from("supplier_accounts").update({ onboarding_status: "approved", approved_at: now, updated_at: now }).eq("id", supplierId);
      if (error) return NextResponse.json({ error: error.message }, { status: 500 });
      ...
      const { data: profile, error: profileError } = await supabaseAdmin.from("service_profiles").update({ status: "active", booking_status: "open" }).eq("business_id", account.business_id).select("id").maybeSingle();
