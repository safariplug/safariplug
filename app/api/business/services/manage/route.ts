...
export async function POST(request: Request) {
  try {
    ...
    if (action === "appointment_status") {
      const allowed=["confirmed","checked_in","in_progress","completed","cancelled","no_show"]; if(!allowed.includes(String(body.status)))return NextResponse.json({error:"Invalid appointment status."},{status:400}); const {data:appointment}=await supabaseAdmin.from("service_appointments").select("id,status,service_profile_id").eq("id",body.appointmentId).eq("service_profile_id",profile.id).maybeSingle(); if(!appointment)return NextResponse.json({error:"Appointment not found."},{status:404}); const {data,error}=await supabaseAdmin.from("service_appointments").update({status:body.status,cancellation_reason:body.status==="cancelled"?(body.reason||null):null}).eq("id",appointment.id).select("*").single(); if(error)return NextResponse.json({error:error.message},{status:400}); await supabaseAdmin.from("service_appointment_status_events").insert({appointment_id:appointment.id,from_status:appointment.status,to_status:body.status,actor_type:"provider",actor_user_id:user.id,note:body.reason||null}); return NextResponse.json({appointment:data});
    }
