...
export async function GET(request: Request) {
  ...
  const itemQuery = supabaseAdmin
    ...
    .select("*, restaurant_menu_item_options(*, restaurant_menu_item_option_values(*))")
    .eq("business_id", businessId)
    .order("sort_order");
  ...
  const publicSettings = settings ? {
    ...
    safari_driver_enabled: settings.safari_driver_enabled,
    customer_driver_enabled: settings.customer_driver_enabled,
    restaurant_delivery_enabled: settings.restaurant_delivery_enabled,
    minimum_order_amount: settings.minimum_order_amount,
    free_delivery_threshold: settings.free_delivery_threshold,
    restaurant_delivery_fee: settings.restaurant_delivery_fee,
    safari_driver_base_fee: settings.safari_driver_base_fee,
    safari_driver_per_km: settings.safari_driver_per_km,
    customer_driver_base_fee: settings.customer_driver_base_fee,
    customer_driver_per_km: settings.customer_driver_per_km,
    preparation_time_minutes: settings.preparation_time_minutes,
  } : null;

  return NextResponse.json({
    settings: supplierView ? settings : publicSettings,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (action === "settings") {
      if (!body.settings || typeof body.settings !== "object") throw new Error("settings is required");
    ...
    if (action === "category") {
      const name = text(body.name, MAX_NAME, "Category name", true);
      const description = text(body.description, MAX_DESCRIPTION, "Category description");
      const sortOrder = integer(body.sortOrder, "sortOrder");
      const { data, error } = await supabaseAdmin.from("restaurant_menu_categories")
        .insert({ business_id: businessId, name, description, sort_order: sortOrder })
        .select().single();
      if (error) return NextResponse.json({ error: error.message }, { status: 400 });
    ...
    if (action === "item") {
      const name = text(body.name, MAX_NAME, "Item name", true);
      const description = text(body.description, MAX_DESCRIPTION, "Item description");
      const imageUrl = text(body.imageUrl, MAX_IMAGE_URL, "Image URL");
      const price = finiteNumber(body.price, "Price", 0, 100000000);
      const currency = String(body.currency ?? "KES").trim().toUpperCase();
      const preparationTimeMinutes = body.preparationTimeMinutes == null ? null : integer(body.preparationTimeMinutes, "preparationTimeMinutes", 1, 1440);
      const sortOrder = integer(body.sortOrder, "sortOrder");
      const categoryId = body.categoryId ? String(body.categoryId) : null;
      if (!CURRENCIES.test(currency)) throw new Error("Currency must be a 3-letter ISO code");

      if (categoryId) {
        const { data: category } = await supabaseAdmin.from("restaurant_menu_categories").select("id").eq("id", categoryId).eq("business_id", businessId).maybeSingle();
        if (!category) return NextResponse.json({ error: "Category not found" }, { status: 404 });
      }

      const { data, error } = await supabaseAdmin.from("restaurant_menu_items")
        .insert({ business_id: businessId, category_id: categoryId, name, description, image_url: imageUrl, price, currency, preparation_time_minutes: preparationTimeMinutes, sort_order: sortOrder })
        .select().single();
      if (error) return NextResponse.json({ error: error.message }, { status: 400 });
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "Invalid menu request" }, { status: 400 });
  }
}
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    const type = body.type === "category" ? "category" : body.type === "item" ? "item" : null;
    ...
    if (type === "category") {
      ...
      const sortOrder = integer(body.sortOrder, "sortOrder");
      const { data, error } = await supabaseAdmin.from("restaurant_menu_categories")
        .update({ name, description, sort_order: sortOrder, active: body.active !== false })
