...
export async function POST(request: Request) {
  ...
  const { businessId, fulfillmentMethod, customerName, customerPhone, customerEmail, deliveryAddress, deliveryLatitude, deliveryLongitude, customerNotes, items } = body;
  ...
  if (["safari_driver", "customer_driver", "restaurant_delivery"].includes(fulfillmentMethod) && !deliveryAddress) {
  ...
  if (subtotal < Number(settings.minimum_order_amount ?? 0)) return NextResponse.json({ error: `Minimum order is ${settings.minimum_order_amount} ${normalized[0] ? (menuItems?.[0]?.currency ?? "KES") : "KES"}` }, { status: 409 });
...
export async function GET(request: Request) {
  ...
  const { data, error } = await supabaseAdmin.from("food_orders").select("*, food_order_items(*)").eq("customer_user_id", user.id).order("created_at", { ascending: false }).limit(50);
