...
export async function POST(request: Request) {
  ...
  if(!user||user.is_anonymous)return NextResponse.json({error:"Sign in to place a food order"},{status:401});
  ...
  const [{data:settings},{data:business}]=await Promise.all([supabaseAdmin.from("restaurant_settings").select("*").eq("business_id",businessId).maybeSingle(),supabaseAdmin.from("businesses").select("id,name,lat,lng").eq("id",businessId).maybeSingle()]);
  ...
  if(driverId){const {data:driver}=await supabaseAdmin.from("driver_profiles").select("id").eq("id",driverId).eq("service_status","active").eq("verification_state","verified").maybeSingle();if(!driver)return NextResponse.json({error:"Selected driver is not available"},{status:409});selectedDriver=driver;}
  const ids=items.map((i:any)=>i.menuItemId).filter(Boolean); const {data:menuItems}=await supabaseAdmin.from("restaurant_menu_items").select("id,name,price,currency,available,active").eq("business_id",businessId).in("id",ids); const byId=new Map((menuItems??[]).map(i=>[i.id,i])); let subtotal=0; const normalized=[];
  for(const item of items){const menu=byId.get(item.menuItemId);const quantity=Number(item.quantity);if(!menu||!menu.active||!menu.available||!Number.isInteger(quantity)||quantity<1)return NextResponse.json({error:"One or more menu items are unavailable"},{status:409});const lineTotal=Number(menu.price)*quantity;subtotal+=lineTotal;normalized.push({menu_item_id:menu.id,item_name:menu.name,unit_price:menu.price,quantity,line_total:lineTotal,notes:item.notes??null});}
