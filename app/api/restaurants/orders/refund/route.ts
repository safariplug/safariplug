...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { data: updatedRefund, error: updateError } = await supabaseAdmin.from("food_order_refunds").update({ provider_reference: reversal.originatorConversationId, refund_reference: reversal.conversationId, updated_at: now }).eq("id", refund.id).eq("status", "processing").select().maybeSingle();
    if (updateError || !updatedRefund) {
      ...
      const { error: markerError } = await supabaseAdmin.from("food_order_refunds").update({ error_message: persistenceMessage, updated_at: now }).eq("id", refund.id).eq("status", "processing");
