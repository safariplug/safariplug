...
async function runTool(name: string, args: Record<string, unknown>, journeyContext: ToolJourneyContext | null = null) {
  ...
  if (name === "search_events") {
    ...
    return (data??[])
      .map((e:any)=>{const rel=Array.isArray(e.cities)?e.cities[0]:e.cities; const start=Date.parse(String(e.start_at)); const end=e.end_at?Date.parse(String(e.end_at)):NaN; const conflicts=busy.filter((w)=>Number.isFinite(start)&&(Number.isFinite(end)?start<w.end&&end>w.start:start>=w.start&&start<w.end)); return {...e,city:rel?.name||null,country:rel?.country||null,href:`/events/${e.id}`,schedule_fit:conflicts.length?"conflict":Number.isFinite(end)?"fits_recorded_schedule":"start_time_clear_duration_unverified",schedule_conflicts:conflicts.map((w)=>w.title)};})
