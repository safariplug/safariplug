...
export default function TransferBookingResultPage() {
  ...
  async function call(action: string, extra: Record<string, unknown> = {}) {
    ...
    if (!response.ok) throw new Error(body?.message || "Unable to verify your transfer booking.");
  ...
  useEffect(() => {
    if (!bookingId) {
      setState("failed");
      setError("We could not identify this transfer booking.");
      return;
    }
    let cancelled = false;
    ...
    const check = async () => {
      ...
      try {
        ...
        if (result.status === "failed") {
          setState("failed");
          setError(result.message || "The transfer booking was not completed.");
          return;
        }
        ...
        if (cancelled) return;
        setState("failed");
        setError(err instanceof Error ? err.message : "Unable to verify your transfer booking.");
      }
    };
  ...
  async function confirmCancellation() {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Unable to cancel transfer.");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <main className="mx-auto max-w-2xl px-6 py-16">
      <div className="rounded-3xl border bg-white p-8 shadow-sm">
        {state === "loading" ? <Status title="Checking your transfer booking…" body="SafariPlug is verifying payment and Hotelbeds confirmation." attempts={attempts} /> : null}
        {state === "pending" ? <Status title={data?.reconciliation === "manual_required" ? "Payment received — supplier confirmation needs review" : "Payment received — transfer is processing"} body={data?.message || "SafariPlug is waiting for payment or supplier confirmation."} attempts={attempts} /> : null}
        {state === "failed" ? <Status title="Transfer booking needs attention" body={error || "The transfer could not be confirmed."} attempts={attempts} /> : null}
        {state === "cancelled" ? (
          <>
            <p className="text-sm font-semibold uppercase tracking-wide text-amber-700">Transfer cancelled</p>
            <h1 className="mt-2 text-3xl font-bold">Hotelbeds transfer</h1>
        ...
        {state === "confirmed" ? (
          <>
            ...
            <p className="text-sm font-semibold uppercase tracking-wide text-green-700">Transfer confirmed</p>
            <h1 className="mt-2 text-3xl font-bold">Your transfer is booked</h1>
            ...
            <div className="mt-6 rounded-2xl border p-5">
              <h2 className="font-semibold">Manage transfer</h2>
              {!cancelPreview ? (
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          ...
          <Link className="rounded-xl border px-4 py-2 font-medium" href="/drivers">Find another transfer</Link>
        </div>
      </div>
    </main>
  );
