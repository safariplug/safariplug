...
export default function CreateEventPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      for (const [label, value] of [["Booking URL", form.booking_url], ["Image URL", form.image_url]] as const) {
        if (value && !/^https:\/\//i.test(value.trim())) throw new Error(`${label} must start with https://`);
      }
      ...
      const { error } = await supabase.from("events").insert({
        title, slug, description, category: form.category, city_id: form.city_id,
        venue_name: form.venue_name.trim() || null, venue_address: form.venue_address.trim() || null,
        start_at: start.toISOString(), end_at: end ? end.toISOString() : null,
        ...
        submitted_by: user.id, status: "pending", featured: false,
      });
  ...
  return <main className="min-h-screen bg-slate-100 text-slate-900">
    ...
    <section className="mx-auto max-w-3xl px-6 py-10"><div className="rounded-3xl bg-white p-8 shadow-sm">
      <p className="text-sm font-black uppercase tracking-widest text-orange-500">Partner Portal</p><h1 className="mt-3 text-3xl font-black">Submit a New Experience</h1><p className="mt-3 text-slate-500">Tell travelers what makes your experience worth discovering. SafariPlug reviews every submission before it appears publicly.</p>
      <form onSubmit={handleSubmit} className="mt-8 space-y-5">
        <div><label className="mb-2 block text-sm font-bold">Experience title *</label><input required maxLength={160} value={form.title} onChange={(e) => update("title", e.target.value)} placeholder="e.g. Sunset Dhow Cruise" className={inputClass} /></div>
        <div><label className="mb-2 block text-sm font-bold">Description *</label><textarea required maxLength={MAX_DESCRIPTION} value={form.description} onChange={(e) => update("description", e.target.value)} placeholder="Describe what travelers will experience..." className={`${inputClass} min-h-32`} /><p className="mt-1 text-right text-xs text-slate-400">{form.description.length}/{MAX_DESCRIPTION}</p></div>
        <div className="grid gap-5 sm:grid-cols-2"><div><label className="mb-2 block text-sm font-bold">Category *</label><select required value={form.category} onChange={(e) => update("category", e.target.value)} className={inputClass}><option value="">Select category</option>{CATEGORIES.map((category) => <option key={category}>{category}</option>)}</select></div><div><label className="mb-2 block text-sm font-bold">City *</label><select required value={form.city_id} onChange={(e) => update("city_id", e.target.value)} className={inputClass} disabled={loadingCities}><option value="">{loadingCities ? "Loading cities..." : "Select city"}</option>{cities.map((city) => <option key={city.id} value={city.id}>{city.name}{city.country ? `, ${city.country}` : ""}</option>)}</select></div></div>
        <div className="grid gap-5 sm:grid-cols-2"><div><label className="mb-2 block text-sm font-bold">Venue</label><input maxLength={200} value={form.venue_name} onChange={(e) => update("venue_name", e.target.value)} placeholder="Venue or meeting point" className={inputClass} /></div><div><label className="mb-2 block text-sm font-bold">Venue address</label><input maxLength={300} value={form.venue_address} onChange={(e) => update("venue_address", e.target.value)} placeholder="Address" className={inputClass} /></div></div>
        ...
        <div><label className="mb-2 block text-sm font-bold">Image URL</label><input maxLength={500} value={form.image_url} onChange={(e) => update("image_url", e.target.value)} placeholder="https://..." className={inputClass} /><p className="mt-2 text-xs text-slate-400">Use a direct HTTPS image URL. A strong landscape photo helps travelers evaluate the listing.</p></div>
        {message && <div aria-live="polite" className={`rounded-xl p-4 text-sm font-bold ${message.includes("successfully") ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700"}`}>{message}</div>}
        <button type="submit" disabled={loading || loadingCities} className="w-full rounded-xl bg-orange-500 py-4 font-black text-white hover:bg-orange-600 disabled:cursor-not-allowed disabled:opacity-50">{loading ? "Submitting..." : "Submit for Review"}</button>
      </form>
    </div></section>
  </main>;
}
