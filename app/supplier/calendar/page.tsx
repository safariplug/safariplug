...
export default function SupplierCalendarPage() {
  ...
  async function load() {
    ...
    if (r.ok) { setStaff(d.staff || []); setAvailability(d.availability || []); setBlockouts(d.blockouts || []); setAppointments(d.appointments || []); if (!selectedStaff && d.staff?.[0]) setSelectedStaff(d.staff[0].id); }
  ...
  return <main className="mx-auto max-w-6xl px-6 py-10">
    ...
    <section className="mt-7 flex flex-wrap items-center gap-3 rounded-2xl border border-black/10 p-4"><label className="text-sm font-medium">Staff calendar <select value={selectedStaff} onChange={e => setSelectedStaff(e.target.value)} className="ml-2 rounded-xl border border-black/15 px-3 py-2">{staff.map(s => <option key={s.id} value={s.id}>{s.display_name}</option>)}</select></label><button disabled={!selectedStaff} onClick={blockTime} className="rounded-full bg-black px-4 py-2 text-sm text-white disabled:opacity-40">Block time</button><span className="text-xs text-black/45">Timezone: Africa/Nairobi (profile timezone)</span></section>
