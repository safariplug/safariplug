...
export default function SupplierOnboardingPage() {
  ...
  const locked = ["submitted", "approved", "live", "rejected"].includes(state.account?.onboarding_status || "");
    ...
    <section className={`mt-6 rounded-2xl border p-5 ${readiness?.ready ? "border-emerald-200 bg-emerald-50/50" : "border-amber-200 bg-amber-50/50"}`}>
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm leading-6 text-black/55">{readiness?.ready ? "Your activation requirements are complete. Submit for review if you have not already done so." : "Complete these items in order. Submit becomes available when every activation requirement below is complete."}</p>
        </div>
