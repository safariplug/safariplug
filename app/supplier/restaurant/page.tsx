...
type Settings = { ordering_enabled: boolean; pickup_enabled: boolean; restaurant_delivery_enabled: boolean; minimum_order_amount: number; restaurant_delivery_fee: number; free_delivery_threshold: number; preparation_time_minutes: number };

const defaults: Settings = { ordering_enabled: true, pickup_enabled: true, restaurant_delivery_enabled: false, minimum_order_amount: 0, restaurant_delivery_fee: 0, free_delivery_threshold: 0, preparation_time_minutes: 30 };
...
export default function RestaurantSupplierPage() {
  ...
  return <main className="mx-auto max-w-4xl px-6 py-10">
    <p className="text-sm uppercase tracking-[.2em] text-black/40">Restaurant supplier</p>
    <h1 className="mt-2 text-4xl font-semibold">Menu & ordering</h1>
    <p className="mt-2 text-black/60">Build the menu customers will see and control how SafariPlug handles pickup and delivery.</p>
    ...
    <section className="mt-8 rounded-2xl border border-black/10 p-5">
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        {(["ordering_enabled", "pickup_enabled", "restaurant_delivery_enabled"] as const).map((key) => <label key={key} className="flex items-center gap-3 rounded-xl bg-black/[.03] p-3 text-sm"><input type="checkbox" checked={settings[key]} onChange={(e) => setSettings((s) => ({ ...s, [key]: e.target.checked }))} />{key === "ordering_enabled" ? "Accept SafariPlug orders" : key === "pickup_enabled" ? "Offer customer pickup" : "Offer restaurant delivery"}</label>)}
        <label className="text-sm"><span className="mb-1 block">Minimum order (KES)</span><input type="number" min="0" value={settings.minimum_order_amount} onChange={(e) => setSettings((s) => ({ ...s, minimum_order_amount: Number(e.target.value) }))} className="w-full rounded-xl border border-black/15 px-3 py-2.5" /></label>
        <label className="text-sm"><span className="mb-1 block">Delivery fee (KES)</span><input type="number" min="0" value={settings.restaurant_delivery_fee} onChange={(e) => setSettings((s) => ({ ...s, restaurant_delivery_fee: Number(e.target.value) }))} className="w-full rounded-xl border border-black/15 px-3 py-2.5" /></label>
        ...
        <label className="text-sm"><span className="mb-1 block">Typical preparation time (minutes)</span><input type="number" min="1" value={settings.preparation_time_minutes} onChange={(e) => setSettings((s) => ({ ...s, preparation_time_minutes: Number(e.target.value) }))} className="w-full rounded-xl border border-black/15 px-3 py-2.5" /></label>
      </div>
      <button disabled={saving} onClick={() => void saveSettings()} className="mt-4 rounded-full bg-black px-5 py-2.5 text-sm text-white disabled:opacity-40">Save ordering settings</button>
    </section>

    <section className="mt-6 rounded-2xl border border-black/10 p-5">
      <h2 className="text-xl font-semibold">Menu categories</h2>
      <div className="mt-4 flex gap-2"><input value={categoryName} onChange={(e) => setCategoryName(e.target.value)} placeholder="e.g. Breakfast, Main Courses, Drinks" className="min-w-0 flex-1 rounded-xl border border-black/15 px-3 py-2.5" /><button disabled={saving} onClick={() => void addCategory()} className="rounded-full bg-black px-4 py-2.5 text-sm text-white disabled:opacity-40">Add category</button></div>
      <div className="mt-4 flex flex-wrap gap-2">{categories.map((category) => <span key={category.id} className="rounded-full bg-black/[.05] px-3 py-1.5 text-sm">{category.name}</span>)}</div>
    </section>

    <section className="mt-6 rounded-2xl border border-black/10 p-5">
      <h2 className="text-xl font-semibold">Add menu item</h2>
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        <input placeholder="Item name" value={item.name} onChange={(e) => setItem((v) => ({ ...v, name: e.target.value }))} className="rounded-xl border border-black/15 px-3 py-2.5" />
        <input type="number" min="0" placeholder="Price in KES" value={item.price} onChange={(e) => setItem((v) => ({ ...v, price: e.target.value }))} className="rounded-xl border border-black/15 px-3 py-2.5" />
        <textarea placeholder="Description" value={item.description} onChange={(e) => setItem((v) => ({ ...v, description: e.target.value }))} rows={3} className="rounded-xl border border-black/15 px-3 py-2.5" />
        <div className="grid gap-3"><select value={item.categoryId} onChange={(e) => setItem((v) => ({ ...v, categoryId: e.target.value }))} className="rounded-xl border border-black/15 px-3 py-2.5"><option value="">No category</option>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select><input type="number" min="1" placeholder="Preparation time (minutes, optional)" value={item.preparationTimeMinutes} onChange={(e) => setItem((v) => ({ ...v, preparationTimeMinutes: e.target.value }))} className="rounded-xl border border-black/15 px-3 py-2.5" /></div>
      </div>
      <button disabled={saving} onClick={() => void addItem()} className="mt-4 rounded-full bg-black px-5 py-2.5 text-sm text-white disabled:opacity-40">Add menu item</button>
    </section>
