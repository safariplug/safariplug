...
export default function RestaurantSupplierPage() {
  ...
  const toggles: Array<[keyof Settings, string]> = [
    ["ordering_enabled", "Accept SafariPlug orders"],
    ["pickup_enabled", "Offer customer pickup"],
    ["safari_driver_enabled", "Offer SafariPlug driver delivery"],
    ["customer_driver_enabled", "Allow customer-arranged driver delivery"],
    ["restaurant_delivery_enabled", "Offer restaurant delivery"],
  ];

  return <main className="mx-auto max-w-4xl px-6 py-10">
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div><p className="text-sm uppercase tracking-[.2em] text-black/40">Restaurant supplier</p><h1 className="mt-2 text-4xl font-semibold">Menu & ordering</h1><p className="mt-2 text-black/60">Build the menu customers will see and control how SafariPlug handles pickup and delivery.</p></div>
      <div className="flex gap-2 text-sm"><Link href="/supplier/onboarding" className="rounded-full border border-black/15 px-4 py-2">Business profile</Link><Link href="/supplier/restaurant/orders" className="rounded-full bg-black px-4 py-2 text-white">Manage orders</Link></div>
    </div>

    <section className="mt-8 rounded-2xl border border-black/10 p-5">
      <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-semibold">Ordering settings</h2><p className="mt-1 text-sm text-black/50">Choose the fulfillment methods your restaurant can reliably support.</p></div><span className={`rounded-full px-3 py-1 text-xs ${settings.ordering_enabled ? "bg-green-500/10 text-green-700" : "bg-black/5 text-black/50"}`}>{settings.ordering_enabled ? "Ordering live" : "Ordering paused"}</span></div>
      <div className="mt-4 grid gap-3 md:grid-cols-2">{toggles.map(([key, label]) => <label key={key} className="flex items-center gap-3 rounded-xl bg-black/[.03] p-3 text-sm"><input type="checkbox" checked={Boolean(settings[key])} onChange={(e) => setSettings((s) => ({ ...s, [key]: e.target.checked }))} />{label}</label>)}
