...
export default function SupplierHomePage() {
  ...
  return <main className="mx-auto max-w-4xl px-6 py-10">
    ...
    {supplier.business && <>
      ...
      {submitted && !approved && readiness && !readiness.ready && <section className="mt-6 rounded-2xl bg-amber-50 p-5 ring-1 ring-amber-200"><h2 className="font-semibold">Review submitted · trust setup still needs attention</h2><p className="mt-1 text-sm leading-6 text-black/55">SafariPlug can review your profile now, but it cannot activate bookings until the remaining trust and payout requirements below are complete.</p><div className="mt-4 space-y-2">{readiness.issues.map((item) => <Link key={item.key} href={item.href} className="flex items-center justify-between rounded-xl bg-white px-4 py-3 text-sm font-medium ring-1 ring-black/5"><span>{item.label}</span><span aria-hidden>→</span></Link>)}</div></section>}{approved && <section className="mt-6 rounded-2xl bg-emerald-50 p-5 ring-1 ring-emerald-200"><h2 className="font-semibold">Approved by SafariPlug</h2><p className="mt-1 text-sm leading-6 text-black/55">Your supplier profile has passed SafariPlug review and the approved business is active.</p>{isAppointment && <div className="mt-4 flex flex-wrap gap-2">{supplier.business?.slug && <Link href={`/services/${supplier.business.slug}`} className="inline-flex rounded-full bg-black px-5 py-3 text-sm font-semibold text-white">View live storefront →</Link>}<Link href="/business/payouts" className="inline-flex rounded-full border border-black/10 bg-white px-5 py-3 text-sm font-semibold">Earnings & payouts →</Link></div>}</section>}
    </>}
  </main>;
}
