...
declare
  ...
  v_start_time time;
  v_end_time time;
  v_staff_verification_current boolean;
begin
  ...
  select exists (
    ...
    where vc.subject_type = 'service_staff'
      and vc.subject_id = v_staff.id
      and vc.status = 'approved'
      and (vc.expires_at is null or vc.expires_at > now())
      and (
        vc.provider = 'human_review'
        or v_staff.identity_liveness_verified_at is not null
      )
  )
  into v_staff_verification_current;

  if not v_staff_verification_current then
    raise exception 'staff_verification_not_current';
  end if;
...
revoke all on function public.create_service_appointment(uuid,uuid,uuid,uuid,text,text,text,timestamptz,text) from public;
grant execute on function public.create_service_appointment(uuid,uuid,uuid,uuid,text,text,text,timestamptz,text) to authenticated;
