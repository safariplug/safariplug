...
-- This migration is intentionally after the existing 20260904050838 migration.
-- It does not create Travel OS tables; those already exist in the live database.

-- Pin search_path for security-sensitive functions. All referenced objects are
-- explicitly schema-qualified, so an empty search_path is safe.
...
-- These are trigger-only functions. Public execution is unnecessary and would
-- expose security-definer entry points. Trigger creation itself remains owned
-- by the schema owner; callers only need table privileges to cause a trigger
-- to fire.
...
-- Keep the existing admin helper callable by authenticated admin middleware.
revoke execute on function public.is_admin() from public, anon;
grant execute on function public.is_admin() to authenticated;

-- Ensure the non-definer trigger functions are not directly callable by API roles.
