...
begin
  ...
  if p_to_status = 'cancelled'
     and v_appointment.payment_status in ('paid','partially_refunded') then
    raise exception 'settled_payment_requires_refund_review';
  end if;
  ...
  insert into public.service_appointment_status_events(appointment_id,from_status,to_status,actor_type,actor_user_id,note)
  values(v_appointment.id,v_from_status,p_to_status,p_actor_type,p_actor_user_id,p_note);
