...
create unique index if not exists ai_scout_runs_one_active_mission_idx
  on public.ai_scout_runs ((lower(btrim(location))), category)
  where status in ('queued','running') and queued_at is not null;

create unique index if not exists ai_discovered_events_title_city_start_unique_idx
  on public.ai_discovered_events ((lower(btrim(title))), (lower(btrim(coalesce(city,'')))), start_at)
  where start_at is not null;

create unique index if not exists ai_discovered_events_source_start_unique_idx
  on public.ai_discovered_events ((lower(btrim(source_url))), start_at)
  where source_url is not null and btrim(source_url) <> '' and start_at is not null;
