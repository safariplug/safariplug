...
begin
  ...
  insert into public.trip_items (
    trip_id, appointment_id, item_kind, position, start_at, end_at, title, notes
  ) values (
    p_trip_id, p_appointment_id, 'personal_service',
    coalesce((select max(position) + 1 from public.trip_items where trip_id = p_trip_id), 0),
    appt.starts_at, appt.ends_at, 'Service appointment', appt.customer_notes
  ) returning * into item;
