...
async function claimIdempotency(params: {
  ...
  if (existing) {
    ...
    if (existing.provider_submission_state === "uncertain") throw new Error("payment_intent_submission_uncertain");
    if (existing.provider_submission_state === "submitted") throw new Error("payment_intent_submission_uncertain");
