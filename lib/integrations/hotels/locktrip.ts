...
type LockTripSearchResult = { hotels?: Array<{ hotelId?: string | number; name?: string; starRating?: number; minPrice?: number; currency?: string; hasFreeCancellation?: boolean }>; searchStatus?: string };
type LockTripRoomsResponse = { hotelId: string; hotelName?: string; packages?: Array<{ quoteId: string; packageId?: string; roomName?: string; roomDescription?: string; mealType?: string; mealDescription?: string; maxOccupancy?: number; amenities?: string[]; price: number; currency: string; pricePerNight?: number; totalNights?: number; isRefundable?: boolean }> };
...
export class LockTripHotelAdapter implements HotelAdapter {
  readonly key = "locktrip" as const; readonly name = "LockTrip"; readonly circuit: CircuitState = "closed";
  private get baseUrl(): string { return env("SAFARIPLUG_HOTEL_LOCKTRIP_BASE_URL") || DEFAULT_BASE_URL; }
  private get nationality(): string { return (env("SAFARIPLUG_HOTEL_LOCKTRIP_NATIONALITY") || DEFAULT_NATIONALITY).toUpperCase(); }
  private async call<T>(tool: string, body: Record<string, unknown>, token?: string): Promise<T> {
    ...
    try {
      ...
      const response = await fetch(`${this.baseUrl}/${tool}`, { method: "POST", headers: { "content-type": "application/json", ...(bearer ? { authorization: `Bearer ${bearer}` } : {}) }, body: JSON.stringify(body), signal: controller.signal, cache: "no-store" });
      const text = await response.text(); let data: unknown = {};
      try { data = text ? JSON.parse(text) : {}; } catch { data = { message: text }; }
      if (!response.ok) { const message = typeof data === "object" && data && "message" in data ? String((data as { message?: unknown }).message) : `LockTrip returned HTTP ${response.status}.`; throw new Error(message); }
      return data as T;
    } finally { clearTimeout(timeout); }
  }
  capabilities(): HotelCapabilities { return { search: true, availability: true, quote: true, hold: false, confirm: true, cancel: false }; }
  ...
  async health(): Promise<HotelHealth> {
    const started = Date.now(); try { await this.call("search_location", { query: "Nairobi" }); return { provider: this.key, status: "healthy", configured: true, contract_implemented: true, reachable: true, authenticated: null, latency_ms: Date.now() - started, last_success_at: new Date().toISOString(), last_error: null, checked_at: new Date().toISOString() }; }
    catch (error) { return { provider: this.key, status: "degraded", configured: true, contract_implemented: true, reachable: false, authenticated: null, latency_ms: Date.now() - started, last_success_at: null, last_error: error instanceof Error ? error.message : "LockTrip health check failed.", checked_at: new Date().toISOString() }; }
  }
  async search(request: HotelSearchRequest): Promise<HotelResult<HotelSearchResponse>> {
    try {
      const locations = await this.call<{ locations?: LockTripLocation[] }>("search_location", { query: request.destination }); const location = locations.locations?.find((item) => item.id != null);
      ...
      for (let attempt = 0; attempt < 8; attempt += 1) { await new Promise((resolve) => setTimeout(resolve, attempt === 0 ? 500 : 750)); result = await this.call<LockTripSearchResult>("get_search_results", { searchKey: started.searchKey, page: 0, size: 100, currency: request.currency || "USD", sortBy: "PRICE_ASC" }); if ((result.searchStatus || "").toUpperCase() === "COMPLETED") break; }
    ...
    } catch (error) { const mapped = asError(error); return { ok: false, error: hotelError(mapped.code, mapped.message, mapped.code === "timeout") }; }
  }
  async getRooms(input: { hotelId: string; searchKey: string; regionId: string; checkIn: string; checkOut: string; rooms: Array<{ adults: number; childrenAges?: number[] }>; currency?: string }) {
    return this.call<LockTripRoomsResponse>("get_hotel_rooms", { hotelId: input.hotelId, searchKey: input.searchKey, startDate: input.checkIn, endDate: input.checkOut, rooms: input.rooms, nationality: this.nationality, regionId: input.regionId, currency: input.currency || "USD" });
  }
  async guestLogin(email: string) { const response = await this.call<{ token: string }>("guest_login", { email }); return response.token; }
  async prepareBooking(token: string, input: Record<string, unknown>) { return this.call<LockTripPrepareResponse>("prepare_booking", input, token); }
