...
registerVerificationAdapter("identity_provider", () => new SumsubVerificationAdapter("identity_provider"));
registerVerificationAdapter("liveness_provider", () => new SumsubVerificationAdapter("liveness_provider"));
...
export function liveVerificationAdapters(): VerificationAdapter[] { return KEYS.map(getVerificationAdapter).filter((adapter) => adapter.key !== "human_review" && adapter.contractImplemented() && adapter.credentialsPresent() && ["healthy", "configured", "degraded"].includes(adapter.status())); }
export async function describeVerificationProviders(): Promise<VerificationProviderDescriptor[]> { return Promise.all(KEYS.map(async (key) => { const adapter = getVerificationAdapter(key); const health = await adapter.health(); const unimplemented = !adapter.contractImplemented(); return { key, name: adapter.name || VERIFICATION_PROVIDER_NAMES[key], status: adapter.status(), configured: adapter.credentialsPresent() || key === "human_review", contract_implemented: adapter.contractImplemented(), capabilities: adapter.capabilities(), reason: unimplemented ? adapter instanceof UnavailableVerificationAdapter ? adapter.reason() : health.last_error || `${adapter.name} has no live contract.` : `${adapter.name} adapter is registered.`, health }; })); }
