    ...
    <form onSubmit={submit} className="mt-8 max-w-4xl" role="search" aria-label="Find anything on SafariPlug">
      <div className="rounded-[1.7rem] border border-white/20 bg-black/45 p-2 shadow-2xl backdrop-blur-xl sm:flex sm:items-center">
        <div className="flex min-w-0 flex-1 items-center gap-3 px-3 sm:px-4">
          ...
          <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Massage in Diani, airport driver, things to do tonight…" className="min-h-14 w-full bg-transparent text-base text-white outline-none placeholder:text-white/40" autoComplete="off" />
        </div>
      ...
      <p className="mt-3 px-2 text-xs leading-5 text-white/45">Search naturally. SafariPlug recognizes common needs and takes you straight to the right area; broader requests open Concierge.</p>
    </form>
  );
}
