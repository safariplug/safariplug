...
const intents = [
  ...
  { words: ["event", "concert", "festival", "party", "nightlife", "tonight", "weekend", "live music", "comedy"], path: "/events", searchable: true },
...
function destinationFor(query: string) {
  ...
  if (!intent) return `/concierge?q=${encodeURIComponent(query)}`;
  if (intent.searchable) return `${intent.path}?search=${encodeURIComponent(query)}`;
  return intent.path;
}
      ...
      <div className="rounded-[1.7rem] border border-white/20 bg-black/45 p-2 shadow-2xl backdrop-blur-xl sm:flex sm:items-center">
        <div className="flex min-w-0 flex-1 items-center gap-3 px-3 sm:px-4">
          ...
          <input
            ...
            placeholder="Massage in Diani, airport driver, things to do tonight…"
            className="min-h-14 w-full bg-transparent text-base text-white outline-none placeholder:text-white/40"
            autoComplete="off"
          />
        </div>
        <button type="submit" disabled={!query.trim()} className="mt-2 w-full rounded-full bg-[#e7c98d] px-6 py-4 text-sm font-black text-[#070708] transition hover:bg-[#f0d9a4] disabled:cursor-not-allowed disabled:opacity-45 sm:mt-0 sm:w-auto">
          Find it →
        </button>
      </div>
      <p className="mt-3 px-2 text-xs leading-5 text-white/45">Search naturally. SafariPlug will take you to the right part of the platform; broader requests open Concierge with your request ready.</p>
    </form>
  );
}
